...
// First story from magical-forest (the complete one)
const glowingFlowersStory = {
  worldName: 'magical-forest',
...
// POST /api/admin/migrate - Migrate existing data to database
  ...
  try {
    ...
    for (const world of worlds) {
      try {
        ...
        console.log(`World ${world.name} already exists or error:`, err);
      }
    }

    // Get magical-forest world ID
    const [magicalForest] = await sql`
      SELECT id FROM worlds WHERE name = 'magical-forest'
    `;
