...
interface GenerateStarterAudioRequest {
  segmentId: string;
  text: string;
  storyId: string;
...
/**
 * Generate TTS audio for a starter story segment.
 * Replaces [CHILD] and [PET] placeholders with SSML pauses for natural splice points.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { segmentId, text, storyId, chapterNumber, segmentOrder } = req.body as GenerateStarterAudioRequest;
  ...
  try {
    ...
    // Upload to Vercel Blob with a predictable path for starter stories
    const storagePath = `starter-audio/${storyId}/ch${chapterNumber}/s${segmentOrder}.mp3`;
    ...
    console.error('Starter audio generation error:', error);
